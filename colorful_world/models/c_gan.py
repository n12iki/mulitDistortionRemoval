        ...
        for epoch_num in range(n_epochs):
            ...
            for data in data_loader:
                ...
                if t % 2 == 1:
                    ...
                    gen_model.train(True)
                    #Gx_example_img = Image.fromarray(np.uint8((bw_img.permute(1, 2, 0).cpu().numpy() / 2 + 0.5) * 256))
                    #print ("size of bw" +str(bw_img.size()))
                    #img_array = bw_img[0].permute(1, 2, 0)
                    #print("img_array: " + str(img_array.size()))
                    #img_array = np.uint8(((img_array.cpu().numpy() / 2.0 + 0.5) * 256))
                    #Gx_example_img = Image.fromarray(img_array)
                    #Gx_example_img=np.array(Gx_example_img)
                    #img_tensor = torch.from_numpy(Gx_example_img).float()
